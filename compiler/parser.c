...
// 表达式节点
表达式列表节点 * 解析器_创建表达式列表节点();
表达式节点 * 解析器_创建表达式节点(uint8_t 节点类型, 表达式节点* 左指针, 表达式节点* 中指针, 表达式节点* 右指针);
    void 表达式列表节点_扩容(表达式列表节点 * 节点指针);
    void 表达式列表节点_追加(表达式列表节点 * 列表节点, 表达式节点 * 待追加节点指针);
 ...
 *                                 语句
 *****************************************************************************/
语句节点 * 解析器_创建语句节点(uint8_t 节点类型, 语句节点* 左指针, 语句节点* 中指针, 语句节点* 右指针) {
    语句节点 * 节点指针 = malloc(sizeof(*节点指针));
    节点指针->开始位置 = 当前单词.位置;
...
语句节点 * 标号语句_解析() {
    ...
    if (当前单词.类型==标号) {
        读入下一单词();
        节点 = 解析器_创建语句节点(标号_节点, NULL, NULL, NULL);
    }else {
        perror("《标号语句》 没有以 label 开始\n");
    }
    if (当前单词.类型 == 标识符) {
        节点->左指针 = 解析器_创建基本表达式节点(当前单词);
    }else {
        perror("《标号语句》 提示词后必选直接根 标识符\n");
    }
    if (当前单词.类型==冒号) {
        读入下一单词();
    }else {
...
// 《如果语句》： if 《表达式》 《裸语句块》 (else 《语句块》|《if语句》)?
语句节点 * 如果语句_解析() {
    语句节点 * 节点;
    if (当前单词.类型 == 如果) {
        读入下一单词();
    ...
    表达式节点 * 条件表达式节点 = 表达式_解析();
    节点 = 解析器_创建语句节点(如果_节点, 条件表达式节点, NULL, NULL);
    节点->中指针 = 裸语句块_解析();
    if (当前单词.类型 == 否则) {
        读入下一单词();
        if (当前单词.类型==如果) {
            节点->右指针 = 如果语句_解析();
        }else if (当前单词.类型==左花括号) {
            节点->右指针 = 语句块_解析();
        }else {
...
// 《表达式列表》 节点初始化
表达式列表节点 * 解析器_创建表达式列表节点() {
    表达式列表节点 * 列表节点指针 = malloc(sizeof(*列表节点指针));
    列表节点指针->开始位置 = 当前单词.位置;
    ...
    列表节点指针->数据区 = (表达式节点**)malloc((列表节点指针->容量)*sizeof(表达式节点*));
}
...
void 表达式列表节点_扩容(表达式列表节点 * 节点指针) {
    ...
    表达式节点 * 新数据区;
    新数据区 = realloc(节点指针->数据区, 新容量 * sizeof(表达式节点*));
    // realloc失败检查
    if (新数据区 == NULL) {
        perror("表达式列表节点 扩容 内存分配失败！\n");
    }
    // 更新容量和数据区指针
    节点指针->容量 = 新容量;
    节点指针->数据区 = 新数据区;
...
表达式节点 * 一元表达式_解析() {
    ...
    case 求尺寸: 
        读入下一单词();
        表达式节点 = 解析器_创建表达式节点(取尺寸_节点, NULL, NULL, NULL);
        if (当前单词.类型==左圆括号) {
            读入下一单词();
        } else {
...
表达式节点 * 后缀运算符_解析(表达式节点 * 目标节点指针) {
    ...
    switch (当前单词.类型) { //设置 节点类型
        case 左方括号: 读入下一单词(); // "[" 《表达式》 "]"
            后缀运算符节点 = 解析器_创建表达式节点(下标运算符_节点, 目标节点指针, NULL, NULL);
            后缀运算符节点->右指针 = 表达式_解析();
            if (当前单词.类型==右方括号) {
                读入下一单词();
            }else {
        ...
        case 左圆括号: 读入下一单词(); // "(" (《赋值表达式》 ((","|"、") 《赋值表达式》)* )? ")"
            后缀运算符节点 = 解析器_创建表达式节点(调用运算符_节点, 目标节点指针, NULL, NULL);
            // 参数列表使用 链表 方式组织，调用运算符节点 左指针 右指针 分别指向链表头尾
            表达式节点 * 当前节点;
            表达式列表节点 * 参数列表节点;
            if (当前单词.类型 != 右圆括号) { //《参数列表》 非空
                参数列表节点 = 解析器_创建表达式列表节点(); //创建列表节点
                后缀运算符节点->右指针 = (表达式节点*)参数列表节点;
                当前节点 = 赋值表达式_解析();
                表达式列表节点_追加(参数列表节点, 当前节点);
                while (当前单词.类型==逗号 || 当前单词.类型==顿号) { //《参数列表》尾 (,|、 《赋值表达式》)*
                    读入下一单词();
                    当前节点 = 赋值表达式_解析(); //创建新《赋值表达式》节点
                    表达式列表节点_追加(参数列表节点, 当前节点);
                }
            }
            if (当前单词.类型==右圆括号) {
        ...
        case 实心句号: 读入下一单词(); // "." 标识符
            后缀运算符节点 = 解析器_创建表达式节点(点运算符_节点, 目标节点指针, NULL, NULL);
            if (当前单词.类型==标识符) {
        ...
        case 当作: 读入下一单词(); // "as" 《类型描述》
            后缀运算符节点 = 解析器_创建表达式节点(强制类型转换_节点, 目标节点指针, NULL, NULL);
            后缀运算符节点->右指针 = 类型描述_解析();
            break;
        // 简单后缀运算符 "->"、"++"、"--"、的尺寸、的类型
        case 单线右单箭头:
            后缀运算符节点 = 解析器_创建表达式节点(解引用_节点, 目标节点指针, NULL, NULL);
            读入下一单词();
            break;
        case 双加号:
            后缀运算符节点 = 解析器_创建表达式节点(递增_节点, 目标节点指针, NULL, NULL);
            读入下一单词();
            break;
        case 双减号:
            后缀运算符节点 = 解析器_创建表达式节点(递减_节点, 目标节点指针, NULL, NULL);
            读入下一单词();
            break;
        case 的尺寸:
            后缀运算符节点 = 解析器_创建表达式节点(取尺寸_节点, 目标节点指针, NULL, NULL);
            读入下一单词();
            break;
        case 的类型:
            后缀运算符节点 = 解析器_创建表达式节点(取类型_节点, 目标节点指针, NULL, NULL);
            读入下一单词();
            break;
        default:
            perror("《后缀运算符》 解析错误!\n");
...
void 解析器_中缀美化打印(FILE * 输出文件, 表达式节点 * 节点指针) {
    ...
    case 逻辑值_节点: fprintf(输出文件,"%s", ((基本表达式节点*)节点指针)->值.逻辑值==true?"true":"false");
        break;
    case 整数_节点: fprintf(输出文件, "%d", ((基本表达式节点*)节点指针)->值.整数); break;
    case 浮点数_节点: fprintf(输出文件, "%f", ((基本表达式节点*)节点指针)->值.浮点数); break;
    case 字符_节点:
        fputc('\'', 输出文件);
    ...
    case 调用运算符_节点:
        ...
        break;
    case 参数列表_节点:
        表达式列表节点 * 列表节点指针 = (表达式列表节点*)节点指针;
        if (列表节点指针->长度 != 0) { //参数列表不为空
            for (int i=0; i<列表节点指针->长度; i++) {
                解析器_中缀美化打印(输出文件, 列表节点指针->数据区[i]);
                fprintf(输出文件, ", ");
            }
            // 最后抹去多余的 ", "
            fprintf(输出文件, "\b\b");
...
void 解析器_前缀美化打印(FILE * 输出文件, 表达式节点 * 节点指针) {
    ...
    case 调用运算符_节点:
        fputc('(', 输出文件);
        解析器_前缀美化打印(输出文件, 节点指针->左指针); //调用过程名
        if (节点指针->右指针 != NULL) {
            解析器_前缀美化打印(输出文件, 节点指针->右指针);
        }
        fputc(')', 输出文件);
