...
// 二元节点，与运算符位置无关(前缀、中缀、后缀)
二元节点 * 抽象语法树_创建二元节点(节点类型 运算符类型,表达式节点 * 左节点指针, 表达式节点 * 右节点指针) {
    二元节点 * 节点指针 = malloc(sizeof(*节点指针));
    节点指针->类型 = 运算符类型;
    节点指针->开始位置.行 = 当前单词.行号; 节点指针->开始位置.列 = 当前单词.列号;
    节点指针->结束位置.行 = 当前单词.行号; 节点指针->结束位置.列 = 当前单词.列号 + 当前单词.单词切片.长度;
    节点指针->左指针 = 左节点指针;
    节点指针->右指针 = 右节点指针;
    return 节点指针;
}
...
// 整数节点
整数节点 * 抽象语法树_创建整数节点(单词信息体 当前单词) {
    整数节点 * 节点指针 = malloc(sizeof(*节点指针));
    节点指针->类型 = 整数;
    节点指针->开始位置.行 = 当前单词.行号; 节点指针->开始位置.列 = 当前单词.列号;
    节点指针->结束位置.行 = 当前单词.行号; 节点指针->结束位置.列 = 当前单词.列号 + 当前单词.单词切片.长度;
    节点指针->值 = 当前单词.字面值;
...
// 整数节点
标识符节点 * 抽象语法树_创建标识符节点(单词信息体 当前单词) {
    标识符节点 * 节点指针 = malloc(sizeof(*节点指针));
    节点指针->类型 = 标识符;
    节点指针->开始位置.行 = 当前单词.行号; 节点指针->开始位置.列 = 当前单词.列号;
    节点指针->结束位置.行 = 当前单词.行号; 节点指针->结束位置.列 = 当前单词.列号 + 当前单词.单词切片.长度;
    节点指针->值 = 当前单词.单词切片;
...
// 基本表达式节点
// 基本表达式节点 * 抽象语法树_创建基本表达式节点(单词信息体 当前单词) {
...
// 《赋值表达式》:= 《条件表达式》 | 《一元表达式列表》 《赋值运算符》 《赋值表达式》  //支持多返回值
// 表达式节点 * 赋值表达式_解析() {
//     static 单词类型 停止单词[] = {分支, 如果, 标识符};
//     表达式节点 * 左指针 = 条件表达式_解析();

//     if (二元中缀优先级表[当前单词.类型]==2){ // 赋值运算符，优先级为2
...
表达式节点 * 基本表达式_解析() {
    ...
    switch (当前单词.类型){
        case 标识符:
            节点指针 = (标识符节点*)抽象语法树_创建标识符节点(当前单词);
            break;
        case 整数字面量:
            节点指针 = (整数节点*)抽象语法树_创建整数节点(当前单词);
            break;
        case 浮点数字面量:
            // 动作
            break;
...
// 美化打印
void 抽象语法树_中缀美化打印(FILE * 输出文件, 表达式节点 * 节点指针) {
    switch (节点指针->类型) {
    case 标识符: unicode切片_文件打印(((标识符节点*)节点指针)->值, 输出文件); break;
    case 整数: fprintf(输出文件, "%d", ((整数节点*)节点指针)->值); break;
    case 加法:
        fprintf(输出文件, "(");
        抽象语法树_中缀美化打印(输出文件, 节点指针->左指针);
        fprintf(输出文件, ")");
        fprintf(输出文件, "+");
        fprintf(输出文件, "(");
        抽象语法树_中缀美化打印(输出文件, 节点指针->右指针);
        fprintf(输出文件, ")");
        break;
    case 乘法:
        fprintf(输出文件, "(");
...
void 抽象语法树_前缀美化打印(FILE * 输出文件, 表达式节点 * 节点指针) {
    switch (节点指针->类型) {
    case 标识符: unicode切片_文件打印(((标识符节点*)节点指针)->值, 输出文件); break;
    case 整数: fprintf(输出文件, "%d", ((整数节点*)节点指针)->值); break;
    case 加法:
        fprintf(输出文件, "+(");
        抽象语法树_前缀美化打印(输出文件, 节点指针->左指针);
        fprintf(输出文件, ", ");
        抽象语法树_前缀美化打印(输出文件, 节点指针->右指针);
        fprintf(输出文件, ")");
        break;
    case 乘法:
        fprintf(输出文件, "×(");
