...
表达式节点 * 解析器_创建后缀表达式节点(表达式节点 * 左节点指针, 节点类型 后缀运算符类型) {
...
// 《后缀运算符》节点，传入 右指针 为空的树根，为其添加右子树
表达式节点 * 解析器_创建后缀运算符节点(表达式节点 * 父节点指针, 节点类型 类型) {
    表达式节点 * 节点指针 = malloc(sizeof(*节点指针));
    节点指针->开始位置 = 当前单词.位置;
    节点指针->结束位置.行 = 当前单词.位置.行; 
    节点指针->结束位置.列 = 当前单词.位置.列 + 当前单词.单词切片.长度;
...
表达式节点 * 后缀表达式_解析() {
    ...
    switch (当前单词.类型) { //设置 节点类型
        case 左方括号:     后缀节点类型 = 下标运算符_节点;   break;
        case 左圆括号:     后缀节点类型 = 调用运算符_节点;   break;
        case 实心句号:     后缀节点类型 = 点运算符_节点;     break;
        case 单线右单箭头: 后缀节点类型 = 右箭头运算符_节点; break;
        case 双加号:       后缀节点类型 = 递增_节点;        break;
        case 双减号:       后缀节点类型 = 递减_节点;        break;
        // 随符集:《后缀表达式》+《一元表达式》+《二元表达式》+《条件表达式》+《赋值表达式》并集
        // = { , 、) ? } + { 二元运算符 } + { 行结束符、 文件结束符 }
        case 逗号: case 顿号: case 右圆括号:
    ...
    //《后缀表达式》:= 《基本表达式》   创建《后缀表达式》节点，后缀复合运算符暂时留空 
    后缀表达式节点指针 = 解析器_创建后缀表达式节点(基本表达式节点指针, 后缀节点类型);
    // (《后缀运算符》)*　//只有 a[i] a(x,y...) a.b 需要右部“复合运算符”
    while (当前单词.类型==左方括号 || 当前单词.类型==左圆括号
            || 当前单词.类型==实心句号 )
    {   // 填充 右指针
        后缀表达式节点指针 = 后缀运算符_解析(后缀表达式节点指针);
    }
    return 后缀表达式节点指针;
}
...
表达式节点 * 后缀运算符_解析(表达式节点 * 后缀表达式节点指针) {
    switch (当前单词.类型) { //设置 节点类型
        case 左方括号: // "[" 《表达式》 "]"
            ...
            break;
        case 左圆括号: // "(" (《赋值表达式》 ((","|"、") 《赋值表达式》)* )? ")"
            读入下一单词();
            if (当前单词.类型==右圆括号) { // 有参数
                // 后缀表达式节点指针->右指针 = 赋值表达式列表_解析();
            } else { //空 参数列表
                后缀表达式节点指针->右指针 = 表达式_解析();
                if (当前单词.类型==右圆括号) {
                    读入下一单词();
...
void 解析器_中缀美化打印(FILE * 输出文件, 表达式节点 * 节点指针) {
    ...
    case 整数_节点: 
        ...
        break;
    case 浮点数_节点:
        fprintf(输出文件, "%f", ((基本表达式节点*)节点指针)->值.浮点数); 
        break;
    case 字符_节点:
        fputc('\'', 输出文件);
