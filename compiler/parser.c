...
// 《表达式》通用节点：一元-中指针；二元-左右指针；三元-左中右指针
...
表达式节点 * 赋值表达式_解析() {
    // 合并为 《赋值表达式》:= 《条件表达式》(《赋值运算符》《赋值表达式》)?
    表达式节点 * 节点;
    节点 = 条件表达式_解析();
    // 《赋值运算符》 := 、+= -= *= ·= ×= /= ÷= %= 、<<= >>= &= ^= |= 其优先级为2
    if (二元中缀优先级表[当前单词.类型]==2) {
    ...
    switch (当前单词.类型) {
    case 右圆括号: case 右方括号:
    case 冒号: case 逗号: case 顿号:
    case 分号: case 换行: case 文件结束:
        break;
    default:
        perror("《赋值表达式》随符集 匹配失败\n");
        break;
    }
    return 节点;
}

// 《条件表达式》:= 《二元表达式》 ("?" 《表达式》 ":" 《条件表达式》)?
表达式节点 * 条件表达式_解析() {
    表达式节点 * 节点;
    // 《二元表达式》
    节点 = 二元表达式_解析(4);
    ...
    if (当前单词.类型==问号) {
        读入下一单词();
        节点 = 解析器_创建表达式节点(条件表达式_节点, 节点, NULL, NULL);
        节点->中指针 = 表达式_解析();
        if (当前单词.类型==冒号) {
            读入下一单词();
            节点->右指针 = 条件表达式_解析();
    ...
    switch (当前单词.类型) {
        ...
        case 分号: case 换行:
        case 文件结束:
        // 合并《赋值表达式》:=《条件表达式》(《赋值运算符》《赋值表达式》)?  产生多余随符集《赋值运算符》
...
// 《前缀运算符》:= & + - ~ !
// 《一元表达式》:= 《前缀运算符》《一元表达式》 | "("《类型描述》")"《一元表达式》
//				  |"sizeof" "(" (《类型描述》|《一元表达式》) ")" | 《后缀表达式》
表达式节点 * 一元表达式_解析() {
    表达式节点 * 表达式节点;
    // 语句节点 * 语句节点;
    switch (当前单词.类型) {
    ...
    case 感叹号: case 逻辑非:
        ...
        表达式节点->左指针 = 一元表达式_解析();
        break;
    // 《一元表达式》:="("《类型描述》")"《一元表达式》 | 《后缀表达式》
    case 左圆括号:
        读入下一单词();
        if (属于类型描述首符集(当前单词.类型)) { //  "("《类型描述》")" 《一元表达式》
            // 语句节点 = 类型描述_解析();
            if (当前单词.类型==右圆括号) {
                读入下一单词();
            }else {
            ...
            // 表达式节点 = 解析器_创建表达式节点(强制类型转换_节点, 语句节点, 一元表达式_解析(), NULL);
        }else { // 《后缀表达式》
            表达式节点 = 解析器_创建表达式节点(强制类型转换_节点, 后缀表达式_解析(), NULL, NULL);
        }
...
表达式节点 * 后缀表达式_解析() {
    ...
    //有 《后缀运算符》*
    while (当前单词.类型==实心句号 || 当前单词.类型==左圆括号
            || 当前单词.类型==左方括号 || 当前单词.类型== 单线右单箭头
...
static uint8_t 单词类型_后缀表达式节点类型表[] = {
    // 《后缀运算符》 [] () . -> ++ --
    [左方括号] = 下标运算符_节点, [左圆括号]     = 调用运算符_节点,
    [实心句号] = 点运算符_节点,   [单线右单箭头] = 右箭头运算符_节点,
    [双加号]   = 递增_节点,      [双减号]       = 递减_节点,
};
...
表达式节点 * 后缀运算符_解析(表达式节点 * 目标节点指针) {
    ...
    switch (当前单词.类型) { //设置 节点类型
        ...
        case 实心句号: 读入下一单词(); // "." 标识符
            ...
            break;
        // 简单后缀运算符 "->"、"++"、"--"
        case 单线右单箭头: case 双加号: case 双减号:
            后缀节点 = 解析器_创建表达式节点(单词类型_后缀表达式节点类型表[当前单词.类型], 目标节点指针, NULL, NULL);
            读入下一单词();
            break;
        default:
            perror("《后缀运算符》 解析错误!\n");
...
void 解析器_中缀美化打印(FILE * 输出文件, 表达式节点 * 节点指针) {
    ...
    case 字符串_节点:
        ...
        break;
    // 《后缀表达式》:= 《基本表达式》 《后缀运算符》*
    case 递增_节点: case 递减_节点:
    case 右箭头运算符_节点:
        解析器_中缀美化打印(输出文件, 节点指针->左指针); //《基本表达式》
        fprintf(输出文件, "%s", 节点类型_提示符表[节点指针->类型]);
        break;
    case 下标运算符_节点: // 《后缀运算符》:= "[" 《表达式》 "]"
        // 先序遍历
        解析器_中缀美化打印(输出文件, 节点指针->左指针);
        fputc('[', 输出文件);
        解析器_中缀美化打印(输出文件, 节点指针->右指针); // 《表达式》
        fputc(']', 输出文件);
        break;
    // 前缀表达式
    // 中缀表达式
...
void 解析器_前缀美化打印(FILE * 输出文件, 表达式节点 * 节点指针) {
    ...
    case 点运算符_节点: case 调用运算符_节点: case 下标运算符_节点:
        fprintf(输出文件, "%s", 节点类型_提示符表[((表达式节点*)节点指针)->类型]);
        if (节点指针->右指针 != NULL) {
            fputc('(', 输出文件);
    ...
    case 条件表达式_节点:
        ...
        fprintf(输出文件, "("); 
        解析器_前缀美化打印(输出文件, 节点指针->左指针); fprintf(输出文件, ", ");
        解析器_前缀美化打印(输出文件, 节点指针->中指针); fprintf(输出文件, ", ");
        解析器_前缀美化打印(输出文件, 节点指针->右指针);
        fprintf(输出文件, ")");
        break;
    // 《赋值表达式》
