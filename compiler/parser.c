...
三元节点 * 解析器_创建表达式节点(uint8_t 节点类型, 三元节点* 左指针, 三元节点* 中指针, 三元节点* 右指针);
    void 表达式列表节点_扩容(表达式列表节点 * 节点指针);
    void 表达式列表节点_追加(表达式列表节点 * 列表节点, 三元节点 * 待追加节点指针);
...
三元节点 * 语句_解析() {
    ...
    switch(当前单词.类型) { // 《语句行》 FIRST首符集？
        ...
        case 中断: 节点 = 中断语句_解析(); break; 
        case 继续: 节点 = 继续语句_解析(); break;
        case 分支: 节点 = 分支语句_解析(); break; //分支 branch if
 ...
 *****************************************************************************/
// 三元指针节点，通常不使用 中指针
三元节点 * 解析器_创建语句节点(uint8_t 节点类型, 三元节点* 左指针, 三元节点* 中指针, 三元节点* 右指针) {
...
// 《如果语句》：if《表达式》 《裸语句块》 (else 《裸语句块》|《if语句》)?
三元节点 * 如果语句_解析() {
    三元节点 * 节点;
    if (当前单词.类型 == 如果) {
        读入下一单词();
    ...
    三元节点 * 条件表达式节点 = 表达式_解析();
    节点 = 解析器_创建语句节点(如果_节点, (三元节点*)条件表达式节点, NULL, NULL);
    节点->中指针 = 裸语句块_解析();
...
三元节点 * 标号语句_解析() {
    ...
    //《标号语句》："label" 标识符 ":" 《语句》?    随符集 \n ; 文件结束
    if (!(当前单词.类型==换行||当前单词.类型==分号||当前单词.类型==文件结束)) {
        节点->右指针 = 语句_解析(); 
    }
    return 节点;
}
...
// 中缀美化打印 (a) + (b)
void 解析器_中缀美化打印(FILE * 输出文件, 三元节点 * 节点指针) {
    switch (节点指针->类型) {
    case 标号_节点:
        fprintf(输出文件, "label ");
...
// 前缀美化打印 (+  a, b)
void 解析器_前缀美化打印(FILE * 输出文件, 三元节点 * 节点指针) {
    switch (节点指针->类型) {
    case 标号_节点: 
        fprintf(输出文件, "label ");
