...
三元节点 * 语句行_解析() {
    三元节点 * 节点;
    switch(当前单词.类型) { // 《语句》FIRST首符集 声明 表达式首符集 分支语句 循环语句 调用
        case 宏: case 类型: case 常量: case 变量: 
        case 标识符: 
        case 标号: case 转到: case 中止: case 继续: 
        case 如果: case 分支:
...
三元节点 * 语句_解析() {
    ...
    switch(当前单词.类型) { // 《语句行》 FIRST首符集？
        ...
        //以标识符、算符开头的语句
        case 标识符:   节点 = 表达式_解析();   break;
        case 左花括号: 节点 = 语句块_解析(); break;
        // case 转到:       跳转语句_解析(); break; //流程控制
...
void 解析器_语句前缀打印(FILE * 输出文件, 三元节点 * 节点指针) {
    ...
    case 如果否则_节点:
        ...
        break;
    case 标号_节点: case 转到_节点: case 中止_节点: case 继续_节点:
        fprintf(输出文件, "%s ", 节点类型_提示符表[节点指针->类型]);
        if (节点指针->左指针 != NULL) {
            解析器_表达式前缀打印(输出文件, 节点指针->左指针);
        }
