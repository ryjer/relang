...
三元节点 * 语句行_解析() {
    ...
    switch(当前单词.类型) { // 《语句》FIRST首符集 声明 表达式首符集 分支语句 循环语句 调用
        case 宏: case 类型: case 常量: case 变量: //macro type const var 标识符
        case 标识符: 
        case 标号: case 转到: case 中止: case 继续: // label goto break continue
        case 如果: case 分支: //if branch
        case 循环: case 当: case 对于: //循环 while for
        case 善后: case 返回:  //《语句行》 FIRST首符集 defer return
            节点 = 语句_解析();
            break;
        default:
            // 报错
