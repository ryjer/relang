...
static uint8_t 二元中缀优先级表[] = {
    // 2 赋值运算符 := 、+= -= *= ·= ×= /= ÷= %= 、<<= >>= &= ∧= |= ∨= ^= ⊕= 右结合
    [冒等于]=2, [加等于]=2, [减等于]=2,
    [星等于]=2, [中间点等于]=2, [叉乘等于]=2, [斜杠等于]=2, [除等于]=2, [百分等于]=2,
    ...
    [加号]=12, [减号]=12,
    // 13 乘除模运算符：* · ×  / ÷  %
    [星号]=13, [中间点]=13, [叉乘号]=13, [斜杠]=13, [除号]=13, [百分号]=13, [取模号]=13,
};
...
三元节点 * 条件表达式_解析() {
    ...
    switch (当前单词.类型) {
        ...
        case 星等于: case 中间点等于: case 叉乘等于: case 斜杠等于: case 除等于: case 百分等于:
...
三元节点 * 二元表达式_解析(uint8_t 优先级) {
    ...
    switch (当前单词.类型){
        // 《二元运算符》首符集
        case 加号: case 减号: case 星号: case 中间点: case 叉乘号:
...
static char* 节点类型_提示符表[] = {
    ...
    [当循环_节点]="while", [直到循环_节点]="until", [原始循环_节点]="for",
