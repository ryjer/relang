...
// 二元运算符节点 * 抽象语法树_创建二元节点(节点类型 运算符类型,表达式节点 * 左节点指针, 表达式节点 * 右节点指针) {
//     二元运算符节点 * 节点指针 = malloc(sizeof(*节点指针));
//     节点指针->类型 = 运算符类型;
...
// 一元运算符节点
// 一元运算符节点 * 抽象语法树_创建一元节点(节点类型 类型, 表达式节点 * 表达式树指针) {
//     一元运算符节点 * 节点指针 = malloc(sizeof(*节点指针));
//     节点指针->类型 = 标识符;
//     节点指针->开始位置.行 = 当前单词.行号; 节点指针->开始位置.列 = 当前单词.列号;
//     节点指针->结束位置.行 = 当前单词.行号; 节点指针->结束位置.列 = 当前单词.列号 + 当前单词.单词切片.长度;
//     节点指针->下指针 = 表达式树指针;
