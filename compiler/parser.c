...
表达式节点 * 后缀表达式_解析() {
    ...
    // 没有《后缀运算符》：《基本表达式》随符集 = 《后缀运算符》首符集+ 《后缀表达式》随符集
    switch (当前单词.类型) {
        case 右圆括号: case 右方括号:
            读入下一单词();
            // 继续
        // 随符集:《后缀表达式》+《一元表达式》+《二元表达式》+《条件表达式》+《赋值表达式》并集
        // = { , 、) ? } + { 二元运算符 } + { 行结束符、 文件结束符 }
        case 逗号: case 顿号:
        case 加号: case 减号: case 星号: case 点乘号: case 叉乘号: case 斜杠: case 除号: case 百分号:
        case 等于号: case 感叹等于: case 不等于号: 
        case 小于号: case 小于等于: case 小于等于号: case 大于号: case 大于等于: case 大于等于号:
        case 双小于: case 双大于: case 与号: case 竖线: case 杨抑号: case 逻辑与: case 逻辑或: case 逻辑异或:
        case 文件结束: case 换行:
        // 没有后缀运算符，直接返回
            return 左节点;
            break;
        default: //加上default，不然编译器警告烦人
            break; 
    }
    //有 《后缀运算符》*
...
static char* 节点类型_提示符表[] = {
    ...
    // 强制类型转换
    // 《二元运算符》
    [加法_节点] = "+", [减法_节点] = "-", [乘法_节点] = "×", [除法_节点] = "÷", [取模_节点] = "mod",
    [等于_节点] = "=", [不等于_节点] = "≠",
