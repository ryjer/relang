...
// 《后缀表达式》 节点，传入左子树，返回右指针为空的节点
表达式节点 * 解析器_创建表达式节点(节点类型 后缀运算符类型, 表达式节点* 左指针, 表达式节点* 右指针) {
...
表达式节点 * 后缀运算符_解析(表达式节点 * 目标节点指针) {
    ...
    switch (当前单词.类型) { //设置 节点类型
        case 左方括号: 读入下一单词(); // "[" 《表达式》 "]"
            后缀节点 = 解析器_创建表达式节点(下标运算符_节点, 目标节点指针, NULL);
            后缀节点->右指针 = 表达式_解析();
            if (当前单词.类型==右方括号) {
                读入下一单词();
            }else {
        ...
        case 左圆括号: 读入下一单词(); // "(" (《赋值表达式》 ((","|"、") 《赋值表达式》)* )? ")"
            后缀节点 = 解析器_创建表达式节点(调用运算符_节点, 目标节点指针, NULL);
            // #################### 参数列表解析 #####################
            if (当前单词.类型 != 右圆括号) { // 有参数
                // 后缀节点->右指针 = 赋值表达式_解析();
            } else { //空 参数列表
        ...
        case 实心句号: 读入下一单词(); // "." 标识符
            后缀节点 = 解析器_创建表达式节点(点运算符_节点, 目标节点指针, NULL);
            if (当前单词.类型==标识符) {
                后缀节点->右指针 = 基本表达式_解析();
            } else {
                // 报错
            ...
            break;
        case 单线右单箭头: 读入下一单词(); 
            后缀节点 = 解析器_创建表达式节点(右箭头运算符_节点, 目标节点指针, NULL);
            break;
        case 双加号: 读入下一单词();
            后缀节点 = 解析器_创建表达式节点(递增_节点, 目标节点指针, NULL);
            break;
        case 双减号: 读入下一单词();
            后缀节点 = 解析器_创建表达式节点(递减_节点, 目标节点指针, NULL);
            break;
        default:
            perror("《后缀运算符》 解析错误!\n");
            break;
...
// 《基本表达式》:= 标识符|逻辑值字面量|整数字面量|浮点数字面量|字符串字面量| "(" 《表达式》 ")"
表达式节点 * 基本表达式_解析() {
    // 解析树
    基本表达式节点 * 节点指针;
    switch (当前单词.类型){
