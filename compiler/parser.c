...
三元节点 * 文件_解析() {
    ...
    // 文件结束
    if (当前单词.类型!=文件结束) {
        perror("《文件》：《声明区》后有其它内容，可能是残缺的过程或函数中的语句\n");
    }
    return 节点;
}
...
三元节点 * 导入声明_解析() {
    ...
    //批量《导入声明》："(" (《导入描述》? \n|;)* ")" )
        ...
        if(当前单词.类型==右圆括号) {
            ...
            perror("《导入描述组》 需要以 右圆括号) 结束\n");
        }
    }else {
        perror("《导入声明》 引导词 类型/type 后应当为 标识符 或 ( 导入描述组 )\n");
    }
    return 节点;
}
...
三元节点 * 导入描述_解析() {
    ...
    if (当前单词.类型 == 字符串字面量) {
        ...
        perror("《导入描述》 应当由 可选别名、导入路径:字符串 组成\n");
    }
    return 节点;
}
...
// 《变量描述》:《标识符列表》 (《类型》 (赋值号 《常量表达式列表》)? | 赋值号 《常量表达式列表》)
三元节点 * 变量描述_解析() {
    三元节点 * 节点;
    ...
    switch(当前单词.类型) {
        ...
        case 等于号: case 冒等于:
            ...
            break;
        default:
            perror("《变量描述》 变量 标识符 后应当指明类型，或者初始化赋值\n");
            break;
    }
    return 节点;
}
...
三元节点 * 结构体类型_解析() {
    ...
    if (当前单词.类型==结构体) {
        ...
        perror("《结构体类型》 必须以 结构体/struct 引导词开始\n");
    }
    if (当前单词.类型==左花括号) {
        读入下一单词();
    }else {
        perror("《结构体类型》 引导词后缺少 左花括号{ \n");
    }
    // (《访问修饰字段》? ,|、|\n)* 首符集 + - 标识符 空行\n
    三元节点 * 字段节点;
    while (当前单词.类型==加号 ||当前单词.类型==减号 ||当前单词.类型==标识符 ||当前单词.类型==换行) {
        if (当前单词.类型!=换行) { //非空行
            ...
            if (当前单词.类型==逗号 || 当前单词.类型==顿号 || 当前单词.类型==换行) {
                ...
                perror("《结构体类型》 字段间缺少分隔符，可用逗号','、顿号'、'、和换行 进行分隔\n");
            }
        }else if (当前单词.类型==换行){
            读入下一单词();
        }else {
...
三元节点 * 共用体类型_解析() {
    ...
    if (当前单词.类型==共用体) {
        ...
        perror("《共用体类型》 必须以 共用体/union 引导词开始\n");
    }
    if (当前单词.类型==左花括号) {
        读入下一单词();
    }else {
        perror("《共用体类型》 引导词后缺少 左花括号{ \n");
    }
    // (《访问修饰字段》? ,|、|\n)* 首符集 + - 标识符 空行\n
    三元节点 * 字段节点;
    while (当前单词.类型==加号 ||当前单词.类型==减号 ||当前单词.类型==标识符 ||当前单词.类型==换行) {
        if (当前单词.类型!=换行) { //非空行
            ...
            if (当前单词.类型==逗号 || 当前单词.类型==顿号 || 当前单词.类型==换行) {
                ...
                perror("《共用体类型》 字段间缺少分隔符，可用逗号','、顿号'、'、和换行 进行分隔\n");
            }
        }else if (当前单词.类型==换行){
            读入下一单词();
        }else {
...
三元节点 * 访问修饰字段_解析() {
    ...
    switch (当前单词.类型){
        ...
        case 标识符:
            ...
            break;
        default:
            perror("《访问修饰字段》 声明没有以 正号'+'、负号'-' 标识符 开头\n");
            break;
        }
    return 节点;
}
...
三元节点 * 枚举体类型_解析() {
    ...
    if (当前单词.类型==枚举体) {
        ...
        perror("《枚举体类型》 必须以 共用体/enum 引导词开始\n");
    }
    if (当前单词.类型==左花括号) {
        读入下一单词();
    }else {
        perror("《枚举体类型》 引导词后缺少 左花括号{ \n");
    }
    // (《枚举成员》? ,|、|\n)*
    三元节点 * 字段节点;
    while (当前单词.类型==标识符 ||当前单词.类型==换行) {
        if (当前单词.类型!=换行) { //非空行
            ...
            if (当前单词.类型==逗号 || 当前单词.类型==顿号 || 当前单词.类型==换行) {
                ...
                perror("《枚举体类型》 字段间缺少分隔符，可用逗号','、顿号'、'、和换行 进行分隔\n");
            }
        }else if (当前单词.类型==换行){
            读入下一单词();
        }else {
...
// 《过程声明》：proc "("《形参声明》")"?  标识符 "("《形参列表》?")" ("("《形参列表》")" | 《类型》)? 《语句块》 
三元节点 * 过程声明_解析() {
    三元节点 * 节点;
    if(当前单词.类型==过程) {
    ...
    if(当前单词.类型==左圆括号 || 当前单词.类型==标识符) { //过程名
        ...
        perror("《过程声明》 引导词后应当是 绑定形参声明 或 过程名\n");
    }
    // 定义体
    if (当前单词.类型==左花括号) {
        节点->右指针 = 语句块_解析();
    }else {
        perror("《过程声明》 过程签名后应当是 左花括号{ 开头的过程定义\n");
    }
    return 节点;
}
...
三元节点 * 函数声明_解析() {
    ...
    if(当前单词.类型==左圆括号 || 当前单词.类型==标识符) { //绑定形参 / 函数名
        ...
        perror("《函数声明》 引导词后应当是 绑定形参声明 或 函数名\n");
    }
    // 定义体
    if (当前单词.类型==左花括号) {
        节点->右指针 = 语句块_解析();
    }else {
        perror("《函数声明》 函数签名后应当是 左花括号{ 开头的过程定义\n");
    }
    return 节点;
}

// 《签名》：( "("标识符:绑定名 《类型》")" )?  函数名:标识符 "(" 《形参列表》? ")" ("("《形参列表》")" | 《类型》)?
签名节点 * 签名_解析() {
