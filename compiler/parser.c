...
表达式节点 * 解析器_创建表达式节点(节点类型 运算符类型, 表达式节点* 左指针, 表达式节点* 中指针, 表达式节点* 右指针) {
...
表达式节点 * 一元表达式_解析() {
    ...
    // 《一元表达式》:= 《前缀运算符》 《一元表达式》
    case 与号: 读入下一单词();
        表达式节点 = 解析器_创建表达式节点(取地址_节点, 一元表达式_解析(), NULL, NULL);
        break;
    case 加号: 读入下一单词();
        表达式节点 = 解析器_创建表达式节点(取正_节点, 一元表达式_解析(), NULL, NULL);
        break;
    case 减号: 读入下一单词();
        表达式节点 = 解析器_创建表达式节点(取负_节点, 一元表达式_解析(), NULL, NULL);
        break;
    case 波浪号: 读入下一单词();
        表达式节点 = 解析器_创建表达式节点(位取反_节点, 一元表达式_解析(), NULL, NULL);
        break; 
    case 感叹号: case 逻辑非:
        读入下一单词();
        表达式节点 = 解析器_创建表达式节点(取反_节点, 一元表达式_解析(), NULL, NULL);
        break;
...
表达式节点 * 后缀运算符_解析(表达式节点 * 目标节点指针) {
    ...
    switch (当前单词.类型) { //设置 节点类型
        ...
        // 简单后缀运算符 "->"、"++"、"--"
        case 单线右单箭头: 读入下一单词(); 
            后缀节点 = 解析器_创建表达式节点(右箭头运算符_节点, 目标节点指针, NULL, NULL);
            break;
        case 双加号: 读入下一单词();
...
void 解析器_中缀美化打印(FILE * 输出文件, 表达式节点 * 节点指针) {
    ...
    case 右箭头运算符_节点:
        ...
        fprintf(输出文件, "%s", 节点类型_提示符列表[节点指针->类型]);
        break;
...
void 解析器_前缀美化打印(FILE * 输出文件, 表达式节点 * 节点指针) {
    ...
    case 递增_节点: case 递减_节点:
    case 右箭头运算符_节点:
        fprintf(输出文件, "%s", 节点类型_提示符列表[((表达式节点*)节点指针)->类型]);
