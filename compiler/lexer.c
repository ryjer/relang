...
单词信息体 转换保留词(单词信息体 待检查单词);       // 将混合的 标识符 中的保留词筛选出来
/******************************************************************************
 *                                   词法分析
...
/******************************************************************************
 * 转换保留词：传入一个“标识符”，检查其是否是保留词并将其转换为对应的保留词
 * 返回值：新的单词信息体，内部的 单词类型可能会被更改
 * 关联：一个已被初始化的 “散列表”
 * 翻译：keyword:保留词
 *****************************************************************************/
...
散列表 * 创建保留词散列表(保留词预编码 * 预编码表指针) {
    ...
    while (当前编码项.保留词[0] != '\0') {
        键切片 =  原始utf8字符串转unicode切片(当前编码项.保留词); // utf8字符串 转 unicode切片
        散列表_插入(新散列表指针, 键切片, 当前编码项.单词类型); // 然后插入 切片键-值 对
        保留词编码表索引 ++;
