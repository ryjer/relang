...
符号 * 插入符号(符号表* 符号表指针, unicode切片 名字) {
    // 检查该 键 是否已存在
...
// 调试
int main() {
    符号表 * 表 = 创建符号表(NULL, 0, 31);
    FILE * 输出文件 = stdout;
    // 插入符号
    fprintf(输出文件, "#### 插入测试 ####\n");
    插入符号(表, 原始utf8字符串转unicode切片("A"));
    插入符号(表, 原始utf8字符串转unicode切片("B"));
    插入符号(表, 原始utf8字符串转unicode切片("abc"));
    符号表_文件序列化打印(表, 输出文件);
    fputc('\n', 输出文件);
    // 序列化打印
    fprintf(输出文件, "#### 查询 ####\n");
    符号节点 * 节点指针;
    节点指针 = 查询符号节点(表, 原始utf8字符串转unicode切片("abc"));
    unicode切片_文件打印(节点指针->符号.名字, 输出文件);
    fputc('\n', 输出文件);
    符号表_文件序列化打印(表, 输出文件);
