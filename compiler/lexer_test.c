...
int main() {
    ...
    // 文件未结束
    while (单词信息.错误 != EOF) {
        printf("{@%d, #%d, %d:%d, \"", 单词信息.计数, 单词信息.行内计数, 单词信息.位置.行, 单词信息.位置.列);
        if (单词信息.单词切片.数据[0] == '\n') {
            fprintf(stdout, "\\n");
        } else {
        ...
        // 条件输出 数字值和错误
        if (单词信息.类型==整数字面量 || 单词信息.类型 == 字符字面量) {printf("%ld %#lx %#lo ", 单词信息.字面值, 单词信息.字面值, 单词信息.字面值);}
        if (单词信息.错误 != 0) {printf("\t错误码:%u ", 单词信息.错误);}
        fputc('\n', stdout);
        单词信息 = 文件_扫描单词(输入文件);
    }
    // 文件结束EOF
    printf("{@%d, #%d, %d:%d, \"EOF", 单词信息.计数, 单词信息.行内计数, 单词信息.位置.行, 单词信息.位置.列);
    printf("\", <%d>, 0} err:%d\n", 单词信息.类型, 单词信息.错误);
    // fprintf(stdout, "\n");
    return 0;
}
