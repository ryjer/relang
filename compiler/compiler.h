...
#define COMPILER_H
    ...
    typedef enum 单词类型{ // 大类: 保留词，标识符, 字面量, 界符, 无效词法记号
        ...
        // 保留词 控制语句
        标号,                 //label
        转到, 调用, 善后, 返回, //goto call defer return
        分支, 如果, 则, 否则,  //分支 branch if then else
        循环, 对于, 直到, 当, 中止, 继续, //循环 loop for until while break continue
        // 基本数据类型 类型定义
 ...
 *****************************************************************************/
    ...
    typedef enum {
        ...
        // 《基本表达式》节点
        标识符_节点, 空标识符_节点, 逻辑值_节点, 整数_节点, 浮点数_节点, 字符_节点, 字符串_节点,
        /* ######################### 声明、语句节点 ######################### */
        语句块_节点, 善后_节点, 返回_节点, 
        标号_节点, 转到_节点, 中止_节点, 继续_节点, 如果_节点, 如果否则_节点, 
    } 节点类型;
